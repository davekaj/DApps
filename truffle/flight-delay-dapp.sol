...
contract FlightDelay is usingOraclize {
	...
	function bookkeeping(uint8 _from, uint8 _to, uint _amount) internal { //bookkeeping is updating the ledger
	...
	//just takes away the % needed to run the account, then go
	function bookAndCalcRemainingPremium() internal returns (uint) { //DK$$$$$ this function gets called a lot of times. it takes the value user sent, takes away a percent for error reserve, and takes away a percent for contract creator
		...
		//%%%DK from account/contract balance, to the account premium, we send V. from premium, to risk fund, we send a little bit. then from premium we send some to reward. then the function returns the final amount of the premium
	...
	//%%%DK so this is created my the person who creates the contrant. so thats why all the funds are put into risk at first, whatever i sent into the contract for risk protection
	// constructor
	function FlightDelay () {
		...
		oraclize_setProof(proofType_TLSNotary | proofStorage_IPFS); //%%%%%%%%%5these are all oraclize contract things. will need to look into this
...
//%%%%%%%%%dont accept a msg.value that isnt 0.1 ether
...
//%%%%%%%%%%% dont accept submissions an hour before thursday night game, and dont accept submissions past the 2017 season, . mith5 need more here but i dunno
...
//%%%%%%%% not sure at the moment if i need this 
		...
		uint policyId = policies.length++; //%%%%%% this is basically just figuring out what number of entrant it is, pushing that to the list, with the address too so theyre linked, and making  policy p = policies[policyid]
