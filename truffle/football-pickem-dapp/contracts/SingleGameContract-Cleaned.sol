...
Things I could Add
-payWinner() function
-getOperatingCostsAndRemainingPool() and all calculation stuff. i need to fix it all so that it actually makes sense. but for now I will just worry about testing it 
-Send a lot of ETHER to start so that it will have room to payout these crappy bets
-logic to check if user entry is OK. i guess this would just be H or A, which is easy now. also would have to dowith DECLINE
- use the decline function
- how does manual payout work
- well, figure out how to get the contract to actually work
...
- how does !send work? , iguess you can literally send ether to that function, and if thats not the case, get out of the function compltely
- i took out oraclize time, as it seems it was being used specifically for the flight delay app. i kept it for whatever, in case I do need it 
