...
var SampleRecipientSuccess = artifacts.require("./SampleRecipientSuccess.sol"); // YES I GET IT A BIT NOW
...
//CREATION
    ...
    it("creation: should create an initial balance of 10000 for the creator", function(done) {
        HumanStandardToken.new(10000, 'Simon Bucks', 1, 'SBX', {from: accounts[0]}).then(function(ctr) {
           // console.log(ctr.balanceOf.call(accounts[0])); yes this consolelog doesnt work but if you pass it in the next then function it does work
...
// this badboy is checking out if name, decimal place, and symbol are correct! :)
    ...
    it("creation: should succeed in creating over 2^256 - 1 (max) tokens", function(done) {
        ...
        assert.isTrue(match);
        done(); //done is a callback (mocha) for at it() function. it helps with testing asyhchronous code
        }).catch(done);
    });
...
//normal transfers without approvals.
    ...
    it("transfers: should transfer 10000 to accounts[1] with accounts[0] having 10000", function(done) {
        var ctr;
        HumanStandardToken.new(10000, 'Simon Bucks', 1, 'SBX', {from: accounts[0]}).then(function(result) { //new deploys a new contract to the network
            ctr = result;
    ...
    });// i will explain in english. you deploy a new contract with new(). then the result is a new function, and you wait till thats done
       // then you call ctr, which is the contract. and do a transfer
       // then you wait for it to be done and then you call the balance of acct1 to see it if worked
       // then you use assert.strictEqual and see if it really worked.
       // then you hit done() and you end this test. if you never hit done you catch 'done' . which is an error i gues
  ...
  //      console.log(`abc ${done}`) --------this here actually printed a long error fucntion. must be part of the "it" function
        var ctr;
        HumanStandardToken.new(10000, 'Simon Bucks', 1, 'SBX', {from: accounts[0]}).then(function(result) {
            ctr = result;
...
//APPROVALS
    ...
    it("approvals: msg.sender should approve 100 to accounts[1]", function(done) {
        ...
        HumanStandardToken.new(10000, 'Simon Bucks', 1, 'SBX', {from: accounts[0]}).then(function(result) {
            ...
            return ctr.allowance.call(accounts[0], accounts[1]);//**********how does allowance get called? is it known. YES IT IN TOKEN.sol........... 
            //no no no! all wrong. human standard token is what we are working with,
            // however human standard token imports standard token , which gives us our kind of basis for
            // the standard token which would be used. however, human standard token adds some functionality that helps humans for dev purposes
            //interstingly the standard token also imports token. i guess they are all connected? */
        }).then(function (result) {
...
/*WHAT IS HAPPENING HERE? ALL THAT IS HAPPENING IS I AM DEPLOYING CONTRACTS. THESE CONTRACTS HAVE METHODS THAT ARE BEING CALLED.
THE METHOD ARE BEING PASSED THE EXACT THINGS THEY SHOULD BE GETTING PASSED. AND THAT IS IT. IF IT WORKS IT WORKS. IF NOT IT DOESNT*/ 
